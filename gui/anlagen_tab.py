from PyQt5.QtWidgets import (
    ...
    QPushButton, QFormLayout, QLineEdit, QComboBox, QGroupBox,
...
# ── Dialog: Anlage erfassen / bearbeiten ─────────────────────────────────────

class AnlagenDialog(QDialog):
    """Popup-Fenster zum Anlegen oder Bearbeiten einer Anlage."""

    def __init__(self, parent=None, vorhandene_daten=None):
        super().__init__(parent)
        self.setWindowTitle("Anlage bearbeiten" if vorhandene_daten else "Neue Anlage")
        self.setMinimumWidth(480)
        self._build_ui()
        self._lade_aktiva_konten()
        if vorhandene_daten:
            self._befuellen(vorhandene_daten)   # Felder mit Bestandsdaten vorausfüllen
    ...
    def _build_ui(self):
        ...
        self.inp_bez    = QLineEdit(placeholderText="z.B. Laptop Dell XPS 15")
        self.inp_datum  = QDateEdit(QDate.currentDate())
        self.inp_datum.setCalendarPopup(True)
        self.inp_datum.setDisplayFormat("dd.MM.yyyy")

        self.inp_kosten  = QDoubleSpinBox(); self.inp_kosten.setRange(0.01, 99_999_999); self.inp_kosten.setSuffix(" €"); self.inp_kosten.setDecimals(2)
        self.inp_nutzung = QSpinBox();       self.inp_nutzung.setRange(1, 100);          self.inp_nutzung.setSuffix(" Jahre"); self.inp_nutzung.setValue(5)
        self.inp_restwert= QDoubleSpinBox(); self.inp_restwert.setRange(0, 99_999_999);  self.inp_restwert.setSuffix(" €"); self.inp_restwert.setDecimals(2)

        self.cmb_konto = QComboBox()
        self.inp_notizen = QTextEdit(placeholderText="Optionale Notizen …")
        self.inp_notizen.setMaximumHeight(80)

        # Live-Anzeige: AfA = (Kosten - Restwert) / Nutzungsdauer
        self.lbl_afa = QLabel("Abschreibung p.a.:  —")
        self.lbl_afa.setStyleSheet("color: #6c63ff; font-weight: bold; font-size: 14px;")
        for signal in (self.inp_kosten.valueChanged,
                       self.inp_nutzung.valueChanged,
                       self.inp_restwert.valueChanged):
            signal.connect(self._update_afa_anzeige)
        ...
        form.addRow("Konto (Aktiva):",      self.cmb_konto)
    ...
    def _lade_aktiva_konten(self):
        """Nur Aktiva-Konten im Dropdown anzeigen (Anlagen gehören immer zu Aktiva)."""
    ...
    def _befuellen(self, d):
        """Bestehende Anlage in die Felder laden."""
        self.inp_bez.setText(d["bezeichnung"])
        self.inp_datum.setDate(QDate.fromString(d["anschaffungsdatum"], "yyyy-MM-dd"))
        ...
        self._update_afa_anzeige()

    def _update_afa_anzeige(self):
        """Lineare AfA-Formel live berechnen und anzeigen."""
        try:
    ...
    def get_daten(self):
        """Gibt alle Formularwerte als Dictionary zurück."""
        return {
    ...
    def accept(self):
        ...
        if d["restwert"] >= d["anschaffungskosten"]:
            QMessageBox.warning(self, "Ungültig",
                                "Restwert muss kleiner als Anschaffungskosten sein.")
            return
        super().accept()


# ── Tab: Anlagenverzeichnis ───────────────────────────────────────────────────
...
class AnlagenTab(QWidget):
    ...
    def _build_ui(self):
        ...
        # Aktions-Buttons
        btn_row = QHBoxLayout()
        self.btn_neu       = QPushButton("+ Neue Anlage");  self.btn_neu.setObjectName("btn_success")
        self.btn_bearbeiten= QPushButton("Bearbeiten");     self.btn_bearbeiten.setObjectName("btn_warning")
        self.btn_loeschen  = QPushButton("Löschen");        self.btn_loeschen.setObjectName("btn_danger")
        self.btn_afa_buchen= QPushButton("AfA jetzt buchen")
        for btn in (self.btn_neu, self.btn_bearbeiten, self.btn_loeschen, self.btn_afa_buchen):
        ...
        self.tabelle.setColumnHidden(0, True)   # ID intern
        ...
        # Gesamtsummen unterhalb der Tabelle
        sum_row = QHBoxLayout()
        self.lbl_ak  = QLabel("Gesamt-AK: 0,00 €")
        self.lbl_afa = QLabel("Gesamt-AfA p.a.: 0,00 €")
        for lbl in (self.lbl_ak, self.lbl_afa):
            lbl.setStyleSheet("font-weight: bold; font-size: 13px; color: #6c63ff;")
        sum_row.addStretch()
        sum_row.addWidget(self.lbl_ak)
        sum_row.addWidget(QLabel("   |   "))
        sum_row.addWidget(self.lbl_afa)
        ...
        self.tabelle.doubleClicked.connect(self._bearbeiten)   # Doppelklick öffnet Bearbeiten-Dialog
    ...
    def lade_anlagen(self):
        ...
        for row, a in enumerate(anlagen):
            items = [
                ...
                QTableWidgetItem(self._iso_zu_de(a["anschaffungsdatum"])),
        ...
        self.tabelle.horizontalHeader().setSectionResizeMode(1, QHeaderView.Stretch)
        self.lbl_ak.setText(f"Gesamt-AK: {gesamt_ak:,.2f} €")
        self.lbl_afa.setText(f"Gesamt-AfA p.a.: {gesamt_afa:,.2f} €")
    ...
    def _neu(self):
        ...
        if dlg.exec_() == QDialog.Accepted:
            ...
            try:
                ...
                self.window().statusBar().showMessage(f"Anlage '{d['bezeichnung']}' angelegt.", 3000)
                self.lade_anlagen()
    ...
    def _bearbeiten(self):
        ...
        if row < 0:
            QMessageBox.information(self, "Hinweis", "Bitte zuerst eine Anlage auswählen.")
            return
        aid = int(self.tabelle.item(row, 0).text())
        # Aktuelle Daten aus der DB holen (nicht aus der Tabelle – sicherer)
        anlage = next((a for a in database.get_alle_anlagen() if a["anlage_id"] == aid), None)
        if not anlage:
            return
        ...
        if dlg.exec_() == QDialog.Accepted:
            ...
            try:
                ...
                self.window().statusBar().showMessage(f"Anlage '{d['bezeichnung']}' aktualisiert.", 3000)
                self.lade_anlagen()
            except Exception as e:
                QMessageBox.critical(self, "DB-Fehler", str(e))
    ...
    def _loeschen(self):
        ...
        antwort = QMessageBox.question(
            self, "Löschen bestätigen", f"Anlage '{bez}' wirklich löschen?",
    ...
    def _afa_buchen(self):
        """Erstellt für jede Anlage mit zugewiesenem Konto eine Abschreibungs-Buchung."""
        anlagen = database.get_alle_anlagen()
        if not anlagen:
            QMessageBox.information(self, "Keine Anlagen", "Es sind noch keine Anlagen vorhanden.")
            return

        antwort = QMessageBox.question(
            self, "AfA buchen",
            f"Jahresabschreibung für {len(anlagen)} Anlage(n) jetzt buchen?",
            QMessageBox.Yes | QMessageBox.No
        )
        if antwort != QMessageBox.Yes:
            return
        ...
        gebucht = 0
        fehler  = []
        datum   = today_date.today().isoformat()
        ...
        for a in anlagen:
            if not a["konto_nr"]:
                ...
                continue
            try:
                # Buchungssatz: Abschreibungen (4800) an Anlage-Konto
        ...
        meldung = f"{gebucht} Buchung(en) erfolgreich erstellt."
        if fehler:
            meldung += "\n\nFehler:\n" + "\n".join(fehler)
        QMessageBox.information(self, "AfA gebucht", meldung)
